 ...
 * Tests for verifying dark mode support in the history page
 */
...
describe('DM-8: Apply dark theme to history page', () => {
  describe('history.ts theme initialization', () => {
    const historyPath = path.join(__dirname, '../../history/history.ts');
    ...
    it('should import initTheme from theme module', () => {
      expect(historyContent).toContain("import { initTheme } from '../shared/theme'");
    });

    it('should call initTheme() on init', () => {
      expect(historyContent).toContain('await initTheme()');
    });

    it('should call initTheme before other initializations', () => {
      const initThemeIndex = historyContent.indexOf('await initTheme()');
      const initLanguageIndex = historyContent.indexOf('await initLanguage()');
      expect(initThemeIndex).toBeLessThan(initLanguageIndex);
    });
  });

  describe('index.html dark mode CSS variables', () => {
    const htmlPath = path.join(__dirname, '../../history/index.html');
    ...
    it('should define dark mode message colors', () => {
      const darkSection = htmlContent.match(/\[data-theme="dark"\]\s*\{[\s\S]*?\}/)?.[0] || '';
      expect(darkSection).toContain('--color-message-user-bg: #1A2A1E');
      expect(darkSection).toContain('--color-message-assistant-bg: #1A221A');
    });
  });
  ...
  describe('index.html uses CSS variables for styling', () => {
    ...
    it('should use --color-surface-default for conversation items', () => {
      expect(htmlContent).toContain('background: var(--color-surface-default)');
    });

    it('should use --color-border-light for conversation item borders', () => {
      expect(htmlContent).toContain('border: 1px solid var(--color-border-light)');
    });

    it('should use --color-border-focus for hover states', () => {
      expect(htmlContent).toContain('border-color: var(--color-border-focus)');
    });

    it('should use --color-text-secondary for meta text', () => {
      expect(htmlContent).toContain('color: var(--color-text-secondary)');
    });

    it('should use --color-text-tertiary for date/time text', () => {
      expect(htmlContent).toContain('color: var(--color-text-tertiary)');
    });

    it('should use --color-text-link for action button hover', () => {
      expect(htmlContent).toContain('color: var(--color-text-link)');
    });

    it('should use --color-surface-hover for action button hover', () => {
      expect(htmlContent).toContain('background: var(--color-surface-hover)');
    });

    it('should use --color-accent-error for danger button', () => {
      expect(htmlContent).toContain('color: var(--color-accent-error)');
    });

    it('should use --color-message-user-bg for user messages', () => {
      expect(htmlContent).toContain('background: var(--color-message-user-bg)');
    });

    it('should use --color-message-assistant-bg for assistant messages', () => {
      expect(htmlContent).toContain('background: var(--color-message-assistant-bg)');
    });
  });
  ...
  describe('auto mode CSS variables', () => {
    ...
    it('should define auto mode message colors in @media query', () => {
      const mediaMatch = htmlContent.match(/@media \(prefers-color-scheme: dark\)\s*\{[\s\S]*?\[data-theme="auto"\]\s*\{[\s\S]*?\}\s*\}/);
      const autoSection = mediaMatch?.[0] || '';
      expect(autoSection).toContain('--color-message-user-bg: #1A2A1E');
      expect(autoSection).toContain('--color-message-assistant-bg: #1A221A');
    });
  });
  ...
  describe('light mode CSS variables (default)', () => {
    ...
    it('should define light mode message-user-bg as #EDF4F0', () => {
      const rootMatch = htmlContent.match(/:root\s*\{[\s\S]*?\}/)?.[0] || '';
      expect(rootMatch).toContain('--color-message-user-bg: #EDF4F0');
    });
  });

  describe('dialog and form styling with CSS variables', () => {
    const htmlPath = path.join(__dirname, '../../history/index.html');
    let htmlContent: string;
    ...
    it('should use CSS variables for confirm dialog background', () => {
      expect(htmlContent).toContain('background: var(--color-surface-default)');
    });

    it('should use CSS variables for confirm dialog border', () => {
      expect(htmlContent).toContain('border: 1px solid var(--color-border-default)');
    });

    it('should use CSS variables for confirm cancel button', () => {
      expect(htmlContent).toContain('background: var(--color-surface-active)');
    });

    it('should use CSS variables for confirm delete button', () => {
      expect(htmlContent).toContain('background: var(--color-accent-error)');
    });

    it('should use CSS variables for search input', () => {
      expect(htmlContent).toContain('background: var(--color-surface-default)');
    });

    it('should use CSS variables for search input focus state', () => {
      expect(htmlContent).toContain('border-color: var(--color-border-focus)');
    });

    it('should use CSS variables for import dialog elements', () => {
      expect(htmlContent).toContain('background: var(--color-accent-success)');
    });

    it('should use CSS variables for import error display', () => {
      expect(htmlContent).toContain('border: 1px solid var(--color-accent-error)');
    });
  });

  describe('search and empty states styling', () => {
    const htmlPath = path.join(__dirname, '../../history/index.html');
    let htmlContent: string;
    ...
    it('should use CSS variables for empty state text', () => {
      expect(htmlContent).toContain('color: var(--color-text-tertiary)');
    });

    it('should use CSS variables for no results state', () => {
      expect(htmlContent).toContain('color: var(--color-text-tertiary)');
    });
  });
});
