 ...
 * Tests for verifying dark mode support in the personas page
 */
...
describe('DM-7: Apply dark theme to personas page', () => {
  describe('personas.ts theme initialization', () => {
    const personasPath = path.join(__dirname, '../../personas/personas.ts');
    let personasContent: string;

    beforeEach(() => {
      personasContent = fs.readFileSync(personasPath, 'utf-8');
    });

    it('should import initTheme from theme module', () => {
      expect(personasContent).toContain("import { initTheme } from '../shared/theme'");
    });

    it('should call initTheme() on DOMContentLoaded', () => {
      expect(personasContent).toContain('await initTheme()');
    });

    it('should call initTheme before other initializations', () => {
      const initThemeIndex = personasContent.indexOf('await initTheme()');
      const initLanguageIndex = personasContent.indexOf('await initLanguage()');
      expect(initThemeIndex).toBeLessThan(initLanguageIndex);
    });
  });

  describe('index.html dark mode CSS variables', () => {
    const htmlPath = path.join(__dirname, '../../personas/index.html');
  ...
  describe('index.html uses CSS variables for styling', () => {
    ...
    it('should use --color-surface-default for persona items', () => {
      expect(htmlContent).toContain('background: var(--color-surface-default)');
    });

    it('should use --color-border-light for persona item borders', () => {
    ...
    it('should use --color-text-secondary for section headers', () => {
      expect(htmlContent).toContain('color: var(--color-text-secondary)');
    });

    it('should use --color-text-tertiary for prompt text', () => {
    ...
    it('should use --color-primary-500 for add button', () => {
      expect(htmlContent).toContain('background: var(--color-primary-500)');
    });

    it('should use --color-primary-600 for add button hover', () => {
      expect(htmlContent).toContain('background: var(--color-primary-600)');
    });
    ...
    it('should use --color-accent-error-hover for danger button hover', () => {
      expect(htmlContent).toContain('background: var(--color-accent-error-hover)');
    });

    it('should use --color-bg-muted for built-in persona background', () => {
      expect(htmlContent).toContain('background: var(--color-bg-muted)');
    });

    it('should use --color-bg-elevated for modal background', () => {
      expect(htmlContent).toContain('background: var(--color-bg-elevated)');
    });

    it('should use --color-border-default for modal border', () => {
  ...
  describe('modal and form styling with CSS variables', () => {
    const htmlPath = path.join(__dirname, '../../personas/index.html');
    let htmlContent: string;
    ...
    it('should use CSS variables for form input styling', () => {
      expect(htmlContent).toContain('background: var(--color-surface-default)');
      expect(htmlContent).toContain('color: var(--color-text-primary)');
    });

    it('should use CSS variables for icon picker', () => {
      expect(htmlContent).toContain('border: 2px solid var(--color-border-light)');
    });

    it('should use CSS variables for selected icon', () => {
      expect(htmlContent).toContain('border-color: var(--color-primary-500)');
      expect(htmlContent).toContain('background: var(--color-primary-50)');
    });

    it('should use CSS variables for char counter', () => {
      expect(htmlContent).toContain('color: var(--color-text-tertiary)');
    });

    it('should use CSS variables for primary button', () => {
      expect(htmlContent).toContain('background: var(--color-primary-500)');
      expect(htmlContent).toContain('color: var(--color-text-inverse)');
    });

    it('should use CSS variables for secondary button', () => {
      expect(htmlContent).toContain('border-color: var(--color-border-default)');
    });
  });
});
